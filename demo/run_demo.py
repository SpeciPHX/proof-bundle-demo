...
def write_docs(run_id: str, metrics_path: Path, receipt_path: Path) -> None:
    Path("AUDIT_SUMMARY.md").write_text(
        "\\n".join([
            ...
            f"- Run ID: {run_id}",
            "",
            "This is a deterministic public-safe demo on synthetic data.",
            "",
            "Proof artifacts:",
...
def main() -> None:
    ...
    gates = [
        GateResult(name="reproducible_run", status="PASS", details={"seed": 0}),
        GateResult(name="baseline_win", status=("PASS" if win else "FAIL"), details={"metric": "rmse", "min_improvement_frac": MIN_IMPROVEMENT_FRAC}),
    ]
    ...
    receipt = make_receipt(
        run_id=run_id,
        inputs={"data_seed": 0, "run_manifest": run_manifest},
        gates=gates,
        metrics=[
            {"name": "mae_baseline", "value": base_mae},
            {"name": "rmse_baseline", "value": base_rmse},
            {"name": "mae_candidate", "value": cand_mae},
            {"name": "rmse_candidate", "value": cand_rmse},
            {"name": "baseline_win", "value": bool(win)},
        ],
    ...
    manifest = make_manifest(
        ...
        notes="Public-safe proof bundle demo with baseline gate and receipts.",
    )
